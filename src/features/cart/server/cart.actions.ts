...
export async function addProductToCart({productId}:{productId:string}) {
    ...
    if(!token){
        throw new Error("Please login first to add products to your cart")
    }
...
export async function updateProductQuantity({productId,count}:{productId:string,count:number}):Promise<CartResponse>{
    const cookieStore = await cookies();
    const token = cookieStore.get("token")?.value || null;
    if(!token){
        throw new Error("You are not logged in")
